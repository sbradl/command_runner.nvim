...
---@field label string Label for displaying to the user. Ignored for a description that only provides `expand`.
---@field filter? fun(string): boolean A filter CommandDescription to further narrow when to offer this command. For an `expand` description it also gates whether `expand` runs at all.
---@field cmd? fun(filename: string, bufnr: integer): Command A function which builds the command details needed for execution based on the file and buffer for which the command should be executed. E.g. the filename could be used to calculate the execution directory or as a starting point to search the project it belongs to with vim.fs.root. Required unless `expand` is given.
---@field expand? fun(filename: string, bufnr: integer): CommandDescription[] Called at pick time (after `filter`) to produce the concrete descriptions to offer in this one's place. Use for command sets only known at pick time, e.g. one entry per task in a project's mise.toml. Each returned description needs a `label` and a `cmd`.
